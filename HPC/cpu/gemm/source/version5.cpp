...
void gemm_v5(const float* A, const float* B, float* C, int M, int K, int N, int lda, int ldb) {
    const uint bk = 32;
    const uint bm = 16;
    const uint bn = 16;
    float As[bm * bk] = {0};
...
void gemm_cpu_version5(const float* A, const float* B, float* C, int M, int K, int N) {
    ...
    if (outfile.is_open()) {
        outfile << "gemm_cpu_v5:" << duration.count() <<
